#!/usr/bin/env python3
"""Weekly gold fair-value calibration using daily Stooq data aggregated to a common week.
Target: XAU/USD Friday/last-observation weekly close.
Predictors: DX.F, US 10Y, US 2Y and VIX weekly last observations, lagged one week.
No interpolation or cross-week filling is used.
"""
...
def url(s):return f'https://stooq.com/q/d/l/?s={s}&d1=20100101&i=d'
def get(u,timeout=60):
 r=urllib.request.Request(u,headers={'User-Agent':'GoldEquilibriumPrice/1.1'})
...
def read_weekly(s):
 # Daily rows are chronological. The final valid observation in each Monday-keyed week is retained.
 out={}
 for r in csv.DictReader(io.StringIO(get(url(s)))):
  try:out[weekkey(r['Date'])]=float(r['Close'])
  except:pass
 return out
def solve(A,b):
...
def main():
 g=read_weekly(TARGET);f={s:read_weekly(s) for s in SER};counts={TARGET:len(g),**{s:len(f[s]) for s in SER}}
 ...
 if len(rows)<260:raise RuntimeError(f'insufficient complete weekly history: {len(rows)}; source_counts={counts}; overlap={len(weeks)}')
